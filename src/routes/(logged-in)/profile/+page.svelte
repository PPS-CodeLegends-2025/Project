<script lang="ts">
  ...
  import { xpService } from '$lib/client/services/xp';
  ...
  let levelInfo = $state(data.levelInfo);
  let error = $state<string | null>(null);
  let activeTab = $state('overview');
  let userModules = $state<any>({ 
    completedModules: [], 
    inProgressModules: [], 
    modules: [] 
  });
  ...
  async function refreshLevelInfo() {
    try {
      levelInfo = await xpService.getLevelInfo(user.id);
    } catch (e) {
      error = "Couldn't refresh progress information";
      console.error(e);
    }
  }
  
  async function fetchUserModuleProgress() {
    try {
      loading = true;
      userModules = await modules.getUserModuleProgress(user.id);
    } catch (e) {
      error = "Couldn't fetch module progress";
      console.error(e);
    } finally {
      loading = false;
    }
		...
		{#if activeTab === 'overview'}
			<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
				<div class="rounded-lg bg-gray-50 p-6 text-center">
					...
					<div class="text-4xl font-bold text-gray-800">{userModules.completedModules?.length || 0}</div>
				</div>
				<div class="rounded-lg bg-gray-50 p-6 text-center">
					<h3 class="mt-0 mb-2 text-base text-gray-600">In Progress Modules</h3>
					<div class="text-4xl font-bold text-gray-800">{userModules.inProgressModules?.length || 0}</div>
				</div>
				<div class="rounded-lg bg-gray-50 p-6 text-center">
			...
			{:else}
				<!-- in-progress modules -->
				{#if userModules.inProgressModules?.length > 0}
					<div class="mt-8">
						<h3 class="mb-4 text-xl font-semibold">In Progress Modules</h3>
						<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
							{#each userModules.inProgressModules as module (module.url)}
								<a 
									href={module.url} 
									...
									<div class="ml-4 flex-1">
										...
										<div class="mt-2">
											<div class="flex items-center justify-between text-sm">
												<span>{module.completedSections.length} of {module.totalSections} sections</span>
				...
				<!-- completed modules -->
				{#if userModules.completedModules?.length > 0}
					<div class="mt-8">
						<h3 class="mb-4 text-xl font-semibold">Completed Modules</h3>
						<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
							{#each userModules.completedModules as module (module.url)}
								<a 
									href={module.url} 
				...
				{#if userModules.completedModules?.length === 0 && userModules.inProgressModules?.length === 0}
